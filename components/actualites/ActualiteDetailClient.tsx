...
export default function ActualiteDetailClient({ slug }: ActualiteDetailClientProps) {
  ...
  useEffect(() => {
    ...
    async function loadArticle() {
      ...
      try {
        const remoteArticle = await getShowcaseArticleBySlug(slug);
        if (!isCancelled) {
        ...
        if (error instanceof ApiClientError && error.status === 404) {
          ...
          setLoadError("Impossible de charger l'article depuis la base.");
        }
      } finally {
        if (!isCancelled) {
          setIsLoading(false);
      ...
      <>
        ...
        <main className="bg-brand-surface min-h-screen flex items-center justify-center px-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm max-w-xl">
            <h1 className="text-xl font-bold text-[#0A2540]">Article introuvable</h1>
